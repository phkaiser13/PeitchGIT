...
* File: ph.nsi
*
* This NSIS script is the primary installer for the ph application. It is the
* single source of truth for all system integration tasks. Its responsibilities include:
*
* 1.  Extracting all application files, including the main executable (ph.exe)
*     and the C++ post-installation assistant (ph-installer.exe).
...
!define MUI_FINISHPAGE_RUN_TEXT "Run ph now"
...
;--------------------------------
Function .onInit
    StrCpy $LogFile "$TEMP\ph_installer.log"
    StrCpy $RemoveUserData "0"
    StrCpy $PathAdded "0"
    FileOpen $0 "$LogFile" w
    ${If} $0 != ""
        FileWrite $0 "ph Installer Log - Session Started$\r$\n"
        FileClose $0
    ${EndIf}
...
Section "ph Core" SecCore
    SectionIn RO
    SetOutPath "$INSTDIR"
    Push "Starting installation of core ph application."
    Call WriteLog
    ...
    Push "Executing C++ assistant: $INSTDIR\bin\ph-installer.exe"
    Call WriteLog

    ExecWait '"$INSTDIR\bin\ph-installer.exe"' $0
    ${If} $0 != 0
...
Section /o "Create Desktop Shortcut" SecDesktop
    CreateShortCut "$DESKTOP\ph.lnk" "$INSTDIR\bin\ph.exe" "" "$INSTDIR\bin\ph.exe" 0
...
Section "Create Start Menu Shortcuts" SecStartMenu
    ...
    CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\ph.lnk" "$INSTDIR\bin\ph.exe"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${SecCore} "Installs the core ph application and runs the dependency checker. (Required)"
    !insertmacro MUI_DESCRIPTION_TEXT ${SecPath} "Adds the application directory to the system PATH for easy command-line access."
    !insertmacro MUI_DESCRIPTION_TEXT ${SecDesktop} "Creates a shortcut to ph on your desktop."
...
Function un.CustomUninstallPage
    ...
    ${NSD_CreateLabel} 0 0 100% 30u \
        "The following ph components will be removed:$\n$\n\
...
;--------------------------------
Function un.onInit
    StrCpy $LogFile "$TEMP\ph_uninstaller.log"
    FileOpen $0 "$LogFile" w
    ${If} $0 != ""
        FileWrite $0 "ph Uninstaller Log - Session Started$\r$\n"
        FileClose $0
    ${EndIf}
...
Section "Uninstall"
    Push "Starting uninstallation process."
    Call un.WriteLog
    ...
    ${If} $RemoveUserData == ${BST_CHECKED}
        ...
        Push "User configuration files removed."
        Call un.WriteLog
    ${EndIf}
    
    DeleteRegKey HKLM "${PRODUCT_UNINST_KEY}"
    DetailPrint "Uninstallation completed."
    Push "Uninstallation process completed successfully."
    Call un.WriteLog
SectionEnd

Function un.onUninstSuccess
    MessageBox MB_OK "ph has been successfully removed from your system."
...
; Version Info
;--------------------------------
VIProductVersion "k8s-prerls-0.0.3-beta.0"
VIAddVersionKey "ProductName" "${PRODUCT_NAME}"
VIAddVersionKey "ProductVersion" "k8s-prerls-0.0.3-beta"
VIAddVersionKey "CompanyName" "${PRODUCT_PUBLISHER}"
VIAddVersionKey "FileDescription" "${PRODUCT_NAME} Installer"
VIAddVersionKey "FileVersion" "k8s-prerls-0.0.3-beta"
